...
export default function ConfirmDialog({
  ...
  confirmDanger = true, showCancel = true, onConfirm, onCancel,
}: Props) {
  ...
  return (
    <Modal transparent visible={visible} animationType="none" statusBarTranslucent onRequestClose={() => dismiss(onCancel)}>
      <Pressable style={s.overlay} onPress={() => showCancel ? dismiss(onCancel) : undefined}>
          ...
          <Pressable>
            <View style={s.iconRow}>
              <View style={[s.iconCircle, confirmDanger ? s.iconCircleDanger : s.iconCircleGold]}>
                <MaterialIcons
                  name={confirmDanger ? 'warning' : 'info-outline'}
                  size={32}
                  color={confirmDanger ? colors.danger : colors.gold}
                />
              </View>
            </View>
            <Text style={s.title}>{title}</Text>
            <Text style={s.message}>{message}</Text>
            <View style={[s.buttons, !showCancel && s.buttonsSingle]}>
              {showCancel && (
...
const s = StyleSheet.create({
  ...
  buttonsSingle: { justifyContent: 'center' },
  btnCancel: {
